...
function RoomsPage() {
  ...
  const getRoomStatus = (room: any) => {
    if (!dateRange?.from || !dateRange?.to) return { status: 'default', color: 'green', text: room.is_available ? 'Available' : 'Full' };
    ...
    // Check bookings
    const overlapping = bookings.filter((b: any) => b.room_id === room.id && b.status !== 'rejected' && b.status !== 'cancelled' && b.status !== 'completed');
  ...
  const getBadgeClass = (color: string) => {
    switch (color) {
      ...
      case 'yellow': return "bg-yellow-400 hover:bg-yellow-500 text-white";
      case 'gray': return "bg-slate-500 hover:bg-slate-600 text-white";
      default: return "";
    }
  };
    ...
    <div className="min-h-screen">
      ...
      <section className="container mx-auto px-4 py-8">
        <div className="mb-8 rounded-xl border bg-card p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold flex items-center gap-2"><CalendarIcon className="h-5 w-5" /> Global Availability Search</h2>
          
          <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-4">
            <div className="lg:col-span-2">
              <label className="mb-1 block text-sm font-medium text-muted-foreground">Select Dates</label>
              <Popover>
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-muted-foreground">Room Name</label>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 border-t pt-4 text-sm">
            <span className="flex items-center gap-1.5 font-medium text-muted-foreground"><Info className="h-4 w-4" /> Legend:</span>
            <span className="flex items-center gap-2"><div className="h-3 w-3 rounded-full bg-green-500"></div> Available</span>
            <span className="flex items-center gap-2"><div className="h-3 w-3 rounded-full bg-yellow-400"></div> Limited</span>
            <span className="flex items-center gap-2"><div className="h-3 w-3 rounded-full bg-red-500"></div> Not Available</span>
            <span className="flex items-center gap-2"><div className="h-3 w-3 rounded-full bg-slate-500"></div> Past/Maintenance</span>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((r: any) => {
              ...
              return (
              <Card key={r.id} className="group overflow-hidden border-border/60 transition-all hover:-translate-y-1 hover:shadow-elegant flex flex-col">
                <div className="aspect-[4/3] overflow-hidden bg-muted relative">
                  ...
                  <div className="absolute top-4 right-4">
                    <Badge variant="secondary" className={`px-2.5 py-0.5 text-xs font-medium shadow-sm ${getBadgeClass(status.color)}`}>
                      {status.text}
                    </Badge>
                  </div>
                </div>
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-4 flex items-end justify-between">
                    ...
                    <Button asChild disabled={status.status === 'booked' || status.status === 'maintenance' || status.status === 'invalid'} className="bg-accent text-accent-foreground hover:bg-accent/90">
                      <Link 
                        to="/book/$roomId" 
                        params={{ roomId: r.id }} 
                        search={{ 
                          check_in: dateRange?.from ? new Date(dateRange.from.getTime() - dateRange.from.getTimezoneOffset() * 60000).toISOString().split('T')[0] : undefined,
                          check_out: dateRange?.to ? new Date(dateRange.to.getTime() - dateRange.to.getTimezoneOffset() * 60000).toISOString().split('T')[0] : undefined
                        }}
                      >
                        Book Now
                      </Link>
