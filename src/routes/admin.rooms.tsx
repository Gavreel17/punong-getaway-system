...
function RoomsTab() {
  ...
  const blank = { name: "", type: "room", description: "", price: 0, capacity: 1, image_url: "", is_available: true, status: "available", maintenance_start: "", maintenance_end: "" };
  ...
  function openEdit(r: any) { setEdit(r); setForm({ ...blank, ...r }); setOpen(true); }
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = { 
      ...
      maintenance_start: form.status === 'maintenance' && form.maintenance_start ? form.maintenance_start : null,
      maintenance_end: form.status === 'maintenance' && form.maintenance_end ? form.maintenance_end : null,
    };
    const { error } = edit
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <Card className="overflow-x-auto">
        <Table>
          <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Type</TableHead><TableHead>Price</TableHead><TableHead>Capacity</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {rooms.map((r: any) => (
              <TableRow key={r.id}>
                ...
                <TableCell>
                  <Badge variant="outline" className="capitalize">{r.status || 'available'}</Badge>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={save} className="grid gap-3 py-2">
              ...
              {form.status === 'maintenance' && (
                <div className="grid grid-cols-2 gap-3 mt-2">
                  <div>
                    ...
                    <Input type="date" required value={form.maintenance_start || ''} onChange={e => setForm({...form, maintenance_start: e.target.value})} />
                  </div>
                  <div>
                    <Label className="text-xs">Maintenance End</Label>
                    <Input type="date" required value={form.maintenance_end || ''} onChange={e => setForm({...form, maintenance_end: e.target.value})} />
                  </div>
                </div>
              )}
            </div>
            ...
            <Button type="submit" className="bg-accent text-accent-foreground hover:bg-accent/90 mt-2">{edit ? "Save changes" : "Add room"}</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
